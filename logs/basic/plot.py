...
def plot_cwnd(setting_name):
    ...
    # set plot data
    df=pd.DataFrame()
    df['second']=df2['second']
    # df['quic'] = df1['cwnd']
    df['tcp'] = df2['cwnd']
    # df["dcm-quic"] = df3["mbps"]

    save_mean(df,setting_name,'cwnd')
    # for s2
    # random_values = np.random.uniform(low=-5, high=2, size=len(df))
    # df["quic"] += random_values
    ...
    # 显示图表
    plt.savefig(f'{setting_name}/cwnd.png')


def plot_rtt(setting_name):
    # df1=read_qperf(f'{setting_name}/quic_quic.json')
    df2=read_tcp(f'{setting_name}/tcp_server.json')
    # df3=read_qperf(f'{setting_name}/dcm_quic.json')
    ...
    # 显示图表
    plt.savefig(f'{setting_name}/rtt.png')
...
if __name__ == '__main__':
    ...
    parser.add_argument("-o", help="cwnd,throughput,rtt", type=str)
    try:
        # 解析命令行参数
        ...
        print(f"setting_value: {setting_value} obs_name: {obs_name}")
    ...
    if obs_name == 'all':
        ...
        plot_cwnd(setting_value)
        plot_rtt(setting_value)
    if obs_name == 'cwnd':
        plot_cwnd(setting_value)
    if obs_name == 'throughput':
        plot_throughput(setting_value)
    if obs_name =='rtt':
        plot_rtt(setting_value)
